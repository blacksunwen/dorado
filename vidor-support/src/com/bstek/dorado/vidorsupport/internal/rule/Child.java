...
public class Child implements IOutputable<OutputContext>, IOutputFiledsable, IOriginalObjectHolder<com.bstek.dorado.idesupport.model.Child> {
	...
	public com.bstek.dorado.vidorsupport.internal.rule.Rule findRule(RuleSet ruleSet, Element element, int clientType) {
		String[] memberRuleIDs = this.getMemberRuleIDs();
		if (memberRuleIDs == null || memberRuleIDs.length == 0) {
			return null;
		}
		
		com.bstek.dorado.vidorsupport.internal.rule.Rule defaultRule = null;
		List<com.bstek.dorado.vidorsupport.internal.rule.Rule> fixedRules = new ArrayList<com.bstek.dorado.vidorsupport.internal.rule.Rule>(5);
		String xmlNodeName = element.getName();
		
		for (String ruleId: memberRuleIDs) {
			com.bstek.dorado.vidorsupport.internal.rule.Rule memberRule = ruleSet.getRule(ruleId);
		...
		for (com.bstek.dorado.vidorsupport.internal.rule.Rule rule: fixedRules) {
			boolean match = true;
