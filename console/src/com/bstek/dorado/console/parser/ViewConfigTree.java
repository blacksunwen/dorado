...
public class ViewConfigTree {
	...
	private Node digView(ParserContext ctx) throws Exception {
		ViewParser viewParser = (ViewParser) ctx.getDoradoContext()
				.getServiceBean("viewParser");
		Node viewNode = new Node(viewParser, "View");
		ctx.properties(viewNode);

		ComponentTypeRegistry registry = (ComponentTypeRegistry) ctx
				.getDoradoContext().getServiceBean("componentTypeRegistry");
		Collection<ComponentTypeRegisterInfo> infos = registry
				.getRegisterInfos();
		for (ComponentTypeRegisterInfo info : infos) {
			Parser parser = xmlParserHelper.getXmlParser(info
					.getClassType());
