(function() {
	 ...
	 * 见:{@link dorado.AttributeSupport#getAttributeWatcher}, {@link dorado.AttributeWatcher}</li>
	 * </ul>
	 * </p>
	 *
	 * @abstract
	...
	dorado.AttributeSupport = $class(/** @scope dorado.AttributeSupport.prototype */
	{
		...
		EVENTS : /** @scope dorado.AttributeSupport.prototype */
		{
			 ...
			 * @param {String} arg.attribute 发生改变的属性名。
			 * @param {Object} arg.value 新的属性值。
		 ...
		 * </p>
		 * @param {Object} [value] 此参数具有多重含义：要设置的属性值。当attr为Object时，此参数将被忽略。
		 * <ul>
		 * <li>当attr为String时，此参数代表要设置的属性值。</li>
		 * <li>当attr为Object时，此参数可代表一组执行选项，见option参数的说明。</li>
		 * </ul>
		 * @param {Object} [options] 执行选项。此参数仅在attr参数为Object时有效。
		 * @param {boolean} [options.skipUnknownAttribute=false] 是否忽略未知的属性。
		 * @param {boolean} [options.tryNextOnError=false] 是否在发生错误后继续尝试后续的属性设置操作。
		 * @param {boolean} [options.preventOverwriting=false] 是否组织本次赋值操作覆盖对象中原有的属性值。
		 * 即对于那些已拥有值(曾经通过set方法写入过值)的属性跳过本次的赋值操作，而只对那些未定义过的属性进行赋值。
		 * @return {dorado.AttributeSupport} 返回对象自身。
		 *
		 * @example
		 * oop.set("label", "Sample Text");
		 * oop.set("visible", true);
		 *
		 * @example
		 * oop.set( {
		 *	 label : "Sample Text",
