(function() {
	...
	dorado.widget.blockview.BlockItemModel = $extend(
			...
			/** @scope dorado.widget.blockview.BlockItemModel.prototype */
			{
				...
				setScrollPos : function(scrollPos) {
					var itemCount = this.getItemCount(), lineCount = this
							.getLineCount();
					if (lineCount > 0) {
	...
	dorado.widget.blockview.ImageBlockRenderer = $extend(
			...
			/** @scope dorado.widget.blockview.ImageBlockRenderer.prototype */
			{
				...
				render : function(dom, arg) {
					...
					if (this.captionProperty && this.labelHeight > 0) {
						...
						var label = (entity instanceof dorado.Entity) ? entity
								.get(this.captionProperty)
					...
					if (this.tipProperty) {
						var tip = (entity instanceof dorado.Entity) ? entity
								.get(this.tipProperty)
						...
						$fly(imageDom)
								.bind(
										...
										function() {
											...
											left = Math
													.round((dom.clientWidth - width) / 2);
											top = Math
													.round((dom.clientHeight
															- labelHeight
					...
					var image = (entity instanceof dorado.Entity) ? entity
							.get(this.imageProperty)
	...
	dorado.widget.AbstractBlockView = $extend(
			...
			/** @scope dorado.widget.AbstractBlockView.prototype */
			{
				...
				createDom : function() {
					...
					$fly(dom)
							.bind("scroll", $scopify(this, this.onScroll))
							.mouseover(
									$scopify(
											this,
											function(evt) {
				...
				refreshDom : function(dom) {
					...
					if (this._blockLayout == "vertical") {
						if (!(lineSize > 0))
							lineSize = (dom.clientWidth - this._horiPadding * 2)
									/ (this._blockWidth + this._horiSpacing);
					} else {
						if (!(lineSize > 0))
							lineSize = (dom.clientHeight - this._vertPadding * 2)
									/ (this._blockHeight + this._vertSpacing);
					}
					this._realLineSize = lineSize = (lineSize < 1) ? 1
							: parseInt(lineSize);
					...
					if (this._blockLayout == "vertical") {
						...
						if (this._lineSize > 0 && this._fillLine) {
							blockWidth = (dom.clientWidth - this._horiPadding
									* 2 - this._horiSpacing * (lineSize - 1))
									/ lineSize;
						}
					} else {
						$container.width(pos[0]).height(dom.clientHeight);
						if (this._lineSize > 0 && this._fillLine) {
							blockHeight = (dom.clientHeight - this._vertPadding
									* 2 - this._vertSpacing * (lineSize - 1))
									/ lineSize;
						}
					}

					this._realBlockWidth = blockWidth = (blockWidth < 1) ? 1
							: parseInt(blockWidth);
					this._realBlockHeight = blockHeight = (blockHeight < 1) ? 1
				...
				_getVisibleBlockRange : function() {
					...
					if (this._blockLayout == "vertical") {
						...
						start = parseInt((scroller.scrollTop - this._vertPadding)
								...
								+ Math
										.round((scroller.clientHeight - this._vertPadding)
												/ blockSize + 0.5);
					} else {
						...
						start = parseInt((scroller.scrollLeft - this._horiPadding)
								...
								+ Math
										.round((scroller.clientWidth - this._horiPadding)
												/ blockSize + 0.5);
					}
					return [ start * this._realLineSize,
							end * this._realLineSize ];
				},
				...
				refreshViewPortContent : function(container) {
					var itemModel = this._itemModel, itemCount = itemModel
							.getItemCount(), scroller = this._dom;
										...
										function(itemDom) {
											if (itemDom.subIndex == self._realLineSize - 1) {
												return ((self._blockLayout == "vertical") ? (itemDom.offsetTop
														+ itemDom.offsetHeight + self._vertSpacing)
														: (itemDom.offsetLeft
																+ itemDom.offsetWidth + self._horiSpacing)) < bottomOrRight;
											}
											return true;
										}, true);
					} else {
					...
					if (this._blockLayout == "vertical") {
						itemModel.setScrollSize(this._dom.clientHeight,
								container.clientHeight);
					} else {
						itemModel.setScrollSize(this._dom.clientWidth,
				...
				refreshItemDoms : function(itemDomContainer) {
					...
					if (hoverDecorator)
						itemDomContainer.insertBefore(hoverDecorator.getDom(),
								itemDomContainer.firstChild);
					if (currentDecorator)
						itemDomContainer.insertBefore(
								currentDecorator.getDom(),
								itemDomContainer.firstChild);
				...
				refreshItemDom : function(itemDomContainer, item, index,
						prepend) {
					var flag = prepend ? -1 : 1;
					if (index < 0)
						flag = -flag;
					index = (this._itemModel.getStartIndex() || 0) + index
							* flag;
					...
					$fly(itemDom).data("item", item).css({
						left : pos[0],
						top : pos[1]
					}).outerWidth(this._realBlockWidth).outerHeight(
							this._realBlockHeight);
					this._itemDomCount++;
					this._lineCount = parseInt(this._itemDomCount
							/ this._realLineSize);
				...
				refreshItemDomData : function(blockDom, item) {
					...
					if (processDefault) {
						var timestamp = (item instanceof dorado.Entity) ? item.timestamp
								: -1;
						if (this._ignoreItemTimestamp || timestamp <= 0
								|| blockDom.timestamp != timestamp) {
							(this._renderer || $singleton(dorado.widget.blockview.DefaultBlockRenderer))
									.render(blockDom, {
										blockView : this,
										data : item
									});
				...
				onScroll : function() {
					...
					function process(p1, p2) {
						...
						scroller._scrollTimerId = $setTimeout(this,
								this.doOnScroll, 300);
					}
				...
				doOnScroll : function() {
					...
					this._itemModel
							.setScrollPos((this._blockLayout == "vertical") ? scroller.scrollTop
									: scroller.scrollLeft);
					this.refreshViewPortContent(this._container);
				},
				...
				_findBlockDom : function(evt) {
					...
					return $DomUtils.findParent(evt.target,
							function(parentNode) {
				...
				onMouseDown : function(evt) {
					...
					if (blockDom || this._allowNoCurrent) {
						var data = blockDom ? $fly(blockDom).data("item")
								: null;
					...
					if (itemIndex < this.startIndex) {
						...
						if (this._blockLayout == "vertical") {
							dom.scrollTop = this._scrollTop = (lineIndex + 1)
									* (this._blockHeight + this._vertSpacing)
									+ this._vertPadding - dom.clientHeight;
						} else {
							dom.scrollLeft = this._scrollLeft = (lineIndex + 1)
									* (this._blockWidth + this._horiSpacing)
									+ this._horiPadding - dom.clientWidth;
						}
					}
					this.doOnScroll();
				},
				...
				findItemDomByPosition : function(pos) {
					...
					if (index >= 0 && index < this._itemModel.getItemCount()) {
						var itemModel = this._itemModel, item = itemModel
								.getItemAt(index);
						var blockDom = this._itemDomMap[itemModel
								.getItemId(item)];
						if (blockDom) {
				...
				setDraggingOverBlockDom : function(blockDom) {
					...
					if (this._draggingOverBlockDom)
						$fly(this._draggingOverBlockDom).removeClass(
								"block-drag-over");
				...
				onDraggingSourceMove : function(draggingInfo, evt) {
					...
					if (dropMode != "onControl") {
						...
						if (blockItem
								&& $fly(blockItem).data("item") == draggingInfo
										.get("object")) {
							blockItem = null;
						}
						...
						if (blockItem) {
							if (dropMode == "insertItems") {
								if (this._blockLayout == "vertical")
									insertMode = (blockItem._dropX < (this._realBlockWidth / 2)) ? "before"
											: "after";
								else
									insertMode = (blockItem._dropY < (this._realBlockHeight / 2)) ? "before"
											: "after";
							} else if (dropMode == "onOrInsertItems") {
								if (this._blockLayout == "vertical") {
									if (blockItem._dropX < 4)
										insertMode = "before";
									else if (blockItem._dropX > (this._realBlockWidth - 4))
										insertMode = "after";
								} else {
									if (blockItem._dropY < 4)
										insertMode = "before";
									else if (blockItem._dropY > (this._realBlockHeight - 4))
										insertMode = "after";
								}
							}
						}
						refObject = blockItem ? $fly(blockItem).data("item")
								: null;
						if (!refObject) {
							targetObject = (dropMode == "onAnyWhere") ? this
									: null;
						} else {
					...
					var accept = (draggingInfo
							.isDropAcceptable(this._droppableTags) && !(dropMode == "onItem" && targetObject == null));
					draggingInfo.set({
					...
					if (accept && eventArg.processDefault) {
						...
						this.showDraggingInsertIndicator(draggingInfo,
								insertMode, blockItem);
					}
				...
				showDraggingInsertIndicator : function(draggingInfo,
						insertMode, blockDom) {
					var insertIndicator = dorado.widget.blockview
							.getDraggingInsertIndicator(this._blockLayout);
					...
					if (insertMode) {
						...
						if (this._blockLayout == "vertical") {
							...
							$insertIndicator.height(blockDom.offsetHeight)
									.left(left - 1).top(blockDom.offsetTop);
						} else {
							var top;
							if (insertMode == "before") {
							...
							$insertIndicator.width(blockDom.offsetWidth).top(
									top - 1).left(blockDom.offsetLeft);
						}
						$insertIndicator.show().appendTo(container);
					} else {
						$insertIndicator.hide().appendTo(
								$DomUtils.getInvisibleContainer());
					}
				},

				onDraggingSourceDrop : function(draggingInfo, evt) {
					this.showDraggingInsertIndicator();
					return dorado.widget.RowList.prototype.onDraggingSourceDrop
