...
@XmlNode(nodeName="SqlProjection")
public class SqlProjection extends BaseProjection {
	...
	public Projection toHibernate(SessionFactory sessionFactory) {
		...
		List<org.hibernate.type.Type> types = new ArrayList<org.hibernate.type.Type>(columns.size());
		...
		org.hibernate.type.Type[] typeArray = types.toArray(new org.hibernate.type.Type[types.size()]);
		
		if (StringUtils.isEmpty(groupBy)) {
			return Projections.sqlProjection(clause, aliasArray, typeArray);
		} else {
			return Projections.sqlGroupProjection(clause, groupBy, aliasArray, typeArray);
		}
	}
}
