...
public class PackFile implements Serializable {
	 ...
	 */
	public transient String sourcePath = null;// should not be used anymore -
												// may deprecate it.
	/**
	...
	// added by Benny
	private boolean eclipsePack200Jar = false;

	public boolean isEclipsePack200Jar() {
		return eclipsePack200Jar;
	}

	public void setEclipsePack200Jar(boolean eclipsePack200Jar) {
		this.eclipsePack200Jar = eclipsePack200Jar;
	}
	 ...
	 * 
	 * @param baseDir
	 *            the baseDirectory of the Fileselection/compilation or null
	 * @param src
	 ...
	 */
	public PackFile(File baseDir, File src, String target,
			List<OsConstraint> osList, int override)
			throws FileNotFoundException {
		this(src, computeRelativePathFrom(baseDir, src), target, osList,
				override, null);
	}
	 ...
	 *            the path relative to the compiletime's basedirectory, use
	 *            computeRelativePathFrom(File, File) to compute this.
	 * @param target
	 *            the path to install the file to
	 * @param osList
	 *            OS constraints
	 ...
	 *             if the specified file does not exist.
	 */
	public PackFile(File src, String relativeSourcePath, String target,
			List<OsConstraint> osList, int override, Map additionals)
			throws FileNotFoundException {
		...
		this.relativePath = (relativeSourcePath != null) ? relativeSourcePath
				.replace(File.separatorChar, '/') : relativeSourcePath;

		this.targetPath = (target != null) ? target.replace(File.separatorChar,
				'/') : target;
		...
		// File.length is undefined for directories - we don't add any data, so
		// don't skip
		// any please!
		if (isDirectory)
			length = 0;
	 ...
	 * 
	 * @param baseDir
	 *            The Base directory that is used to search for the files. This
	 *            is used to build the relative path's
	 * @param src
	 *            file which this PackFile describes
	 * @param target
	 *            the path to install the file to
	 ...
	 * 
	 * @param baseDir
	 *            The Base Directory to build the relative path from
	 * @param file
	...
	public static String computeRelativePathFrom(File baseDir, File file) {
		...
		} catch (Exception x)// don't throw an exception here. return null
								// instead!
		{
			// if we cannot build the relative path because of an error, the
			// developer should be
			// informed about.
			x.printStackTrace();
		}
	...
	public void setPreviousPackFileRef(String previousPackId,
