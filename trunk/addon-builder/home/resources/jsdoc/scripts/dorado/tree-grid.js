...
(function () {
    ...
    dorado.widget.treegrid.TreeColumnCellRenderer = $extend([dorado.widget.grid.DefaultCellRenderer, dorado.widget.tree.TreeNodeRenderer], {getLabel:function (node, arg) {
        ...
        if (!dom.firstChild || dom.firstChild.className != "d-tree-node") {
            $fly(dom).empty().xCreate({tagName:"DIV", className:"d-tree-node", content:[{tagName:"LABEL", doradoType:"tree-button", className:"button"}, {tagName:"LABEL", className:"label", whiteSpace:"no-wrap"}]});
    ...
    dorado.widget.treegrid.TreeColumnCellEditor = $extend(dorado.widget.grid.DefaultCellEditor, {hideCellContent:false, shouldShow:function () {
        ...
        $fly(dom).css({left:lp, top:t}).outerWidth(w - (lp - l)).outerHeight(h);
        if (control) {
    ...
    dorado.widget.TreeGrid = $extend([dorado.widget.AbstractGrid, dorado.widget.AbstractTree], {$className:"dorado.widget.AbstractTreeGrid", ATTRIBUTES:{treeColumn:{writeBeforeReady:true}, currentNode:{skipRefresh:true, getter:function (p) {
    ...
    }}, currentEntity:{setter:function (p, currentEntity) {
        ...
        if (this._treeColumn) {
            ...
            if (!col._editor) {
                col._editor = new dorado.widget.treegrid.TreeColumnCellEditor();
            }
        }
        $invokeSuper.call(this, arguments);
    ...
    dorado.widget.treegrid.InnerTreeGrid = $extend(dorado.widget.grid.AbstractInnerGrid, {ATTRIBUTES:{currentNode:{skipRefresh:true, setter:function (p, currentNode) {
        if (this._currentNode == currentNode) {
            return;
        }
        this._currentNode = currentNode;
        if (this._selectionMode == "singleRow") {
            this.replaceSelection(null, currentNode);
        }
    ...
    }, refreshItemDoms:TreePrototype.refreshItemDoms, _nodeInserted:TreePrototype._nodeInserted, _nodeRemoved:TreePrototype._nodeRemoved, _insertChildNodes:TreePrototype._insertChildNodes, _removeChildNodes:TreePrototype._removeChildNodes, _nodeExpanded:TreePrototype._nodeExpanded, _nodeCollapsed:TreePrototype._nodeCollapsed});
})();
dorado.widget.DataTreeGrid = $extend([dorado.widget.TreeGrid, dorado.widget.DataTree], {$className:"dorado.widget.DataTreeGrid", ATTRIBUTES:{autoCreateColumns:{defaultValue:true}}, createRootNode:function () {
    return new dorado.widget.tree.DataBindingNode({bindingConfig:{}});
}, onCellValueEdited:null, initColumns:dorado.widget.DataGrid.prototype.initColumns, refresh:function () {
