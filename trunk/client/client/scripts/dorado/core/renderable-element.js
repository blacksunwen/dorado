 ...
 */
dorado.RenderableElement = $extend(
		dorado.AttributeSupport,
		/** @scope dorado.RenderableElement.prototype */
		{
			...
			ATTRIBUTES : /** @scope dorado.RenderableElement.prototype */
			{
				 ...
				 * @attribute
				 * 
				 * @example // 当我们需要为DOM元素指定背景色和字体颜色时可以使用这样的style
				 *          renderable.set("style", { color : "yellow",
				 *          backgroundColor : "blue" });
				 * 
				 * @example renderable.set("style", "color: yellow;
			...
			doSet : function(attr, value) {
				...
				if (this._rendered && this._ignoreRefresh < 1 && def
						&& !def.skipRefresh) {
					dorado.Toolkits.setDelayedAction(this,
							"$refreshDelayTimerId", this.refresh, 50);
				}
			},
			 ...
			 * <p>
			 * 此方法只会在对象第一次渲染时执行一次。
			 * 所以一般而言，不应该在此放置初始化DOM元素的代码。例如那些设置DOM元素的颜色、字体、尺寸的代码。
				...
				if (dom.nodeType != 3) {
					...
					if (this._style) {
						...
						if (typeof this._style == "string") {
							// 此段处理不能用jQuery.attr("style",
							// style)替代，原因是该方法会覆盖DOM原有的inliine style设置。
							var map = {};
			 ...
			 * @protected
			 * @param {boolean}
			 *            [forced] 是否强制重设对象的尺寸，忽略对于宽高值是否发生过改变的判断。
			...
			getRealWidth : function() {
				return (this._realWidth == null) ? this._width
						: this._realWidth;
			},
			...
			getRealHeight : function() {
				return (this._realHeight == null) ? this._height
						: this._realHeight;
			},
			...
			getDom : function() {
				...
				// if (!this._rendered) this.refreshDom(this._dom);
				return this._dom;
			},
			 ...
			 * @param {HTMLElement}
			 *            containerElement
			 *            作为容器的DOM元素。如果此参数为空，将以document.body作为容器。
			 * @param {HTMLElement}
			 *            [nextChildElement]
			 *            指定新的DOM元素要在那个子元素之前插入，即通过此参数可以指定新的DOM元素的插入位置。
			 */
			render : function(containerElement, nextChildElement) {
				this.doRenderToOrReplace(false, containerElement,
						nextChildElement);
			},
			 ...
			 * @param {boolean}
			 *            delay
			 *            是否允许此次refresh动作延时执行。设置成true有利于系统对refresh动作进行优化处理。
			 */
			refresh : function(delay) {
				if (!this._rendered
						|| (!this._attached && this.renderUtilAttached))
					return;
				if (delay) {
					dorado.Toolkits.setDelayedAction(this,
							"$refreshDelayTimerId", function() {
								dorado.Toolkits.cancelDelayedAction(this,
										"$refreshDelayTimerId");
