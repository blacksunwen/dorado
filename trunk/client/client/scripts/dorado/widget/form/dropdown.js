...
dorado.widget.DropDown = $extend(dorado.widget.Trigger, /** @scope dorado.widget.DropDown.prototype */ {
	...
	ATTRIBUTES: /** @scope dorado.widget.DropDown.prototype */ {
		 ...
		 * </p>
		 * <p>
		 * 此属性的值应该是一个表达式，其基本格式为：<pre>被赋值属性名1=下拉项中的属性名1,被赋值属性名2=下拉项中的属性名2,...</pre><br>
		 ...
		 * </p>
		 * <p>
		 * 注意：在定义“下拉项中的属性名”时可以使用<pre>$this</pre>这样一个特殊的属性名，表示直接将选中的下拉项对象作为数值赋值给目标Entity的某属性。
		 * </p>
		 * <p>
		 * 注意：<pre>$default</pre>可以作为一个特殊的段落，代表系统默认的处理下拉选中项的动作。
		 * </p>
		 * @type String
		 * @attribute
		 *
		 ...
		 * dropdown.set("propertyMaps", "propertyId=id,propertyName=name");
		 *
		 * @example
		 * // 此表达式表示将下拉选中项的id和name属性中的值分别赋值给目标Entity的id和name属性。
		 * dropdown.set("propertyMaps", "id,name");
		 propertyMaps: {},
		 */
		//=====
	...
	EVENTS: /** @scope dorado.widget.DropDown.prototype */ {
		 ...
		 * @param {dorado.widget.AbstractEditor} arg.editor 下拉框此刻关联的编辑器。
		 ...
		 * @param {Object} arg 事件参数。
		 * @param {dorado.widget.AbstractEditor} arg.editor 下拉框此刻关联的编辑器。
		 * @param {Object} #arg.selectedValue 被选中的数值。
		 ...
		 * 如果要在此事件中定义或修改下拉框选中的值，请将新的值写入arg参数的selectedValue属性中。
		 * 如果将arg.selectedValue的值设置为undefined，则表示本次关闭下拉框时不向编辑框中传递任何值。
		 * @param {boolean} #arg.processDefault=true 是否在事件结束后继续执行系统默认的赋值动作。
		 * @return {boolean} 是否要继续后续事件的触发操作，不提供返回值时系统将按照返回值为true进行处理。
		 * @event
	...
	open: function(editor) {
		...
		function getBoxCache(win) {
			...
			try {
				if (win.dorado) {
					...
					if (!boxCache) win.dorado._DROPDOWN_BOX_CACHE = boxCache = {};
				}
			} 
			catch (e) {
				// do nothing
		...
		if (!box) {
			...
			this._view.registerInnerControl(box);
			...
			if (eventArg.processDefault && eventArg.selectedValue !== undefined) {
				...
				try {
					...
					/*
					 ...
					 if (this._propertyMaps && selectedValue && selectedValue instanceof Object &&
					 targetEntity &&
					 targetEntity instanceof Object) {
					 var propertyMaps = this._propertyMaps, maps = [];
					 propertyMaps = propertyMaps.split(',');
					 for (var i = 0; i < propertyMaps.length; i++) {
					 var map = propertyMaps[i], index = map.indexOf('=');
					 if (index >= 0) {
					 maps.push({
					 writerProperty: map.substring(0, index),
					 ...
					 value = (selectedValue instanceof dorado.Entity) ? selectedValue.get(map.readProperty) : selectedValue[map.readProperty];
					 }
...
dorado.widget.DropDownBox = $extend([dorado.widget.Control, dorado.widget.FloatControl], /** @scope dorado.widget.DropDownBox.prototype */ {
	...
	ATTRIBUTES: /** @scope dorado.widget.DropDownBox.prototype */ {
		...
		hideAnimateType: {
			defaultValue: "safeSlide" //"safeSlide"
		},
	...
	init: function(editor) {
		if (this._control) this._control.render(this.getContainerElement());
	},
