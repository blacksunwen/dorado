(function() {
	...
	dorado.widget.blockview.DefaultBlockRenderer = $extend(dorado.Renderer,/** @scope dorado.widget.blockview.DefaultBlockRenderer.prototype */ {
		...
		/**
		 * 渲染。
		 * @param {HTMLElement} dom 块对应的DOM对象。
	 ...
	 * @param {Object} [options] 包含配置信息的JSON对象。<br>
	...
	dorado.widget.AbstractBlockView = $extend(dorado.widget.ViewPortList, /** @scope dorado.widget.AbstractBlockView.prototype */ {
		...
		ATTRIBUTES: /** @scope dorado.widget.AbstractBlockView.prototype */ {
			...
			/**
			 * 每行或列显示的块的个数。
			 * <p>如果不设置此属性或设置为0表示有系统自动判断每行或列显示的块个数。</p>
			 * @type int
			 * @attribute
			 */
			lineSize: {},
		...
		EVENTS: /** @scope dorado.widget.AbstractBlockView.prototype */ {
			 ...
			 * @param {HTMLElement} arg.dom 块对应的DOM对象。
			 * @param {Object|dorado.Entity} arg.data 块对应的数据实体。
		...
		createDom: function() {
			...
			var container = this._container = $DomUtils.xCreateElement({
				tagName: "DIV",
				style: {
					position: "relative",
					overflow: "hidden"
		...
		refreshDom: function(dom) {
			...
			if (this._blockLayout == "vertical") {
				if (!(lineSize > 0)) lineSize = (dom.clientWidth - this._horiPadding * 2) / (this._blockWidth + this._horiSpacing);
			} else {
				if (!(lineSize > 0)) lineSize = (dom.clientHeight - this._vertPadding * 2) / (this._blockHeight + this._vertSpacing);
			}
		...
		setHoverBlock: function(itemDom) {
			...
			if (itemDom && this._currentBlock != itemDom) {
				decorator.set({
					width: itemDom.offsetWidth + this._blockDecoratorSize * 2,
					height: itemDom.offsetHeight + this._blockDecoratorSize * 2,
					style: {
	...
	dorado.widget.blockview.getDraggingInsertIndicator = function(direction) {
		...
		if (indicator == null) {
			indicator = $DomUtils.xCreateElement({
				tagName: "div",
				className: "d-block-dragging-insert-indicator-" + code
			});
