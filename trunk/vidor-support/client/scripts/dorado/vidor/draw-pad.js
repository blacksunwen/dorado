		...
		getDockType: function (mouseX, mouseY) {
			...
			if (regionControl) {
				...
				if (_.isArray(children)) {
					children.each(function (control) {
						doGet(control);
					});
				} else {
		...
		setProperty: function (nid, name, value) {
			...
			if (!(value instanceof Object) && control) {
				try {
					...
					if (attr.writeBeforeReady) return;
					if (attr.readOnly) return;

					var option = {};
					option[name] = value;
					control.set(option, {tryNextOnError: true});
	...
	$(document).keydown(function (event) {
		...
		if (processDefault && (event.ctrlKey || event.metaKey)) {
			switch (event.keyCode) {
				case 83:  // Ctrl+S
					var winEditor = window.parent.editor;
